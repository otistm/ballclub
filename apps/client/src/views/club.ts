...
import { CLASSES, nextScenario, rankTeams, type League, type Team } from '@ballclub/engine';
import { esc } from '../ui/dom.js';
import { M, pctS, readable } from '../ui/format.js';
...
function mandateText(L: League, me: Team): string {
  ...
  const gp = me.w + me.l;
  if (L.phase === 'draft') return 'Twelve rounds. Do not come back with a roster full of first basemen.';
...
export function viewClub(): string {
  ...
  if (L.phase === 'draft') {
    s += `<div class="panel"><div class="eyebrow">Draft in progress</div>
      <p class="dim">Your board is open in the Market tab.</p>
      <button class="btn primary" style="margin-top:11px" data-act="tab" data-k="market">Go to the board</button></div>`;
    ...
    if (n) {
      s += `<div class="panel">
        ...
        <div style="display:flex;align-items:center;gap:12px">
          <div style="flex:0 0 44px;height:44px;display:grid;place-items:center;background:${n.opp.color};clip-path:polygon(50% 0,100% 16%,100% 74%,50% 100%,0 74%,0 16%)">
            <svg viewBox="0 0 24 24" style="width:24px;height:24px;fill:${readable(n.opp.color)}"><path d="${GLYPH[n.opp.glyph] || GLYPH.anvil}"/></svg>
          </div>
          <div style="flex:1">
  ...
  s += `<div class="panel paper">
    <div class="eyebrow">From the board</div>
  ...
  /* franchise feed */
  const feed = L.log.slice(-9).reverse().filter((x) => !x.draft);
  if (feed.length) {
    s += `<div class="eyebrow">Around the club</div><div class="panel feed">`;
    feed.forEach((f) => {
      s += `<div class="fitem"><div class="w">W${f.w || '-'}</div><div class="x">${esc(f.txt)}</div></div>`;
    });
    s += `</div>`;
  }
  return s;
